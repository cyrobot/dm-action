/**
 * Created by robert on 14-9-27.
 */
/*
 item.jd.com Compressed by uglify
 Author:keelii
 Date: 2014-09-25 4:50:20 [PM]
 */
...
function strtotime(t, e) {
    ...
    function s(t) {
        ...
        if ("next" === s || "last" === s)i(s, a, c); else if (!n)return!1;
        return!0
    }

    var a, n, o, r, c, d, l, p, m, u;
    if (!t)return null;
    ...
    if (r = e ? new Date(1e3 * e) : new Date, c = {sun: 0, mon: 1, tue: 2, wed: 3, thu: 4, fri: 5, sat: 6}, d = {yea: "FullYear", mon: "Month", day: "Date", hou: "Hours", min: "Minutes", sec: "Seconds"}, p = "(years?|months?|weeks?|days?|hours?|minutes?|min|seconds?|sec|sunday|sun\\.?|monday|mon\\.?|tuesday|tue\\.?|wednesday|wed\\.?|thursday|thu\\.?|friday|fri\\.?|saturday|sat\\.?)", m = "([+-]?\\d+\\s" + p + "|" + "(last|next)\\s" + p + ")(\\sago)?", n = t.match(RegExp(m, "gi")), !n)return!1;
    for (u = 0, l = n.length; l > u; u++)if (!s(n[u]))return!1;
    return r.getTime() / 1e3
}
function mktime() {
    var t = new Date, e = arguments, i = 0, s = ["Hours", "Minutes", "Seconds", "Month", "Date", "FullYear"];
    for (i = 0; s.length > i; i++)if (e[i] === void 0)e[i] = t["get" + s[i]](), e[i] += 3 === i; else if (e[i] = parseInt(e[i], 10), isNaN(e[i]))return!1;
...
function monthDay(t, e) {
    var i, s, a, n = new Date(1e3 * t), o = new Date(1e3 * e), r = n.getFullYear(), c = n.getMonth() + 1, d = n.getDate(), l = o.getFullYear(), p = o.getMonth() + 1, m = o.getDate(), u = "", h = 12 * (l - r) + p - c;
    return i = Math.floor(h / 12), s = h % 12, s = 0 === s - Math.floor(s) ? s + 1 : s, a = (mktime(0, 0, 0, p, m, l) - mktime(0, 0, 0, p, d, l)) / 86400, 0 > a && (s -= 1), 0 > s && (i -= 1), i > 0 && (u += i + "\u5e74"), s > 0 && (u += s + "\u4e2a\u6708"), u
}
function el_top_middle(t, e, i) {
    ...
    var o = n.docHeight() != n.clientHeight() ? 16 : 0, r = $(document).scrollTop(), c = (n.clientHeight() - a) / 2 + r, d = (n.clientWidth() - o - s) / 2;
    t.css({left: d, top: c})
}
function showBaitiaoFq() {
    ...
    n.append(s), $(document.body).prepend(n), $("#Fqfk_Tip").show(), el_top_middle($("#c04tip"), 502, 358), $.ajax({url: "http://fa.jd.com/loadFa.js?aid=2_163_5702", dataType: "script", cache: !0}), $(".Tip_Close").click(function () {
        $("#c04tip").remove()
    })
}
function showNaBaitiao() {
    ...
    i.append(s), $(document.body).prepend(i), $("#Fqfk_Tip").show(), el_top_middle($("#c04tip"), 502, 358), $(".Tip_Close").click(function () {
        $("#c04tip").remove()
    }), $.ajax({url: "http://fa.jd.com/loadFa.js?aid=2_163_5480", dataType: "script", cache: !0})
}
...
function getSuitInfoService(t) {
    if (null !== t.packResponseList && t.packResponseList.length > 0) {
        ...
        $("#favorable-suit").show(), $("#favorable-suit .mc").html(suit_TPL.tabs.process(t) + suit_TPL.cons.process(t)), $("#favorable-suit .mc").Jtab({event: e >= t.packResponseList.length ? "mouseover" : "click", delay: 300, compatible: !0, currClass: "scurr"}, function (t, e) {
            var i = e.attr("packprice"), s = e.attr("data-cat"), a = e.attr("packlistprice"), n = e.attr("discount"), o = e.find(".fitting-price"), r = e.find(".orign-price"), c = e.find(".fitting-saving"), d = e.find(".infos .p-name a,.infos .btns a"), l = [G.sku];
            ...
            }), G.getNewCLK({name: "yhtz", action: "0", csku: l, sku: G.sku})
...
function onlineService(t, e, i) {
    ...
    o = s(o);
    var r = $("#store-selector .text").text(), c = $("#store-prompt strong").html(), d = r + "\uff08" + c + "\uff09";
    d = s(d), e = s(e);
    var l = $("span[class^='star']").attr("class").replace("star sa", ""), p = 100;
    try {
    ...
    var v = "http://" + i + "/index.action?", b = {pid: a, price: g, stock: d, score: l, commentNum: p, imgUrl: h, wname: n, advertiseWord: o, seller: e, evaluationRate: u, recent: f, code: t, area: s($("#store-selector .text").text()), size: s($("#choose-version .dd .selected a").text()), services: s($("#summary-service .dd").html())};
...
(function (t) {
    "object" == typeof pageConfig.product && (t.sku = pageConfig.product.skuid, t.key = pageConfig.product.skuidkey, t.url = pageConfig.product.href, t.src = pageConfig.product.src, t.name = pageConfig.product.name, t.mdPerfix = 1 == pageConfig.product.type ? "CR" : "GR", t.mbPerfix = 1 == pageConfig.product.type ? "3C" : "GM", t.mp = pageConfig.product.mp, t.jp = pageConfig.product.jp, t.cat = pageConfig.product.cat, t.orginSku = pageConfig.product.orginSkuid || t.sku, t.isJd = 1e9 > t.sku, t.isPop = t.sku > 1e9, t.isCellphone = 655 == pageConfig.product.cat[2] || 6881 == pageConfig.product.cat[2] || 6882 == pageConfig.product.cat[2], t.isArray = function (t) {
        ...
        var e, i, s, a, n = t.indexOf("?"), o = t.substr(0, n), r = t.substr(n + 1), c = r.split("&"), d = c.length, l = {};
        for (e = 0; d > e; e++)i = c[e].split("="), s = i[0], a = i[1], l[s] = a;
        return{url: o, param: l}
        ...
        }, $.ajax({url: "http://club.jd.com/ProductPageService.aspx?method=GetCommentSummaryBySkuId&referenceId=" + t + "&callback=getCommentCount", dataType: "script", cache: !0}), void 0)
        ...
        for (var i = $(t).parents(e), s = i.find(".master input").attr("wmeprice"), a = i.find("input:checked"), n = a.length, o = i.find(".infos .res-totalprice"), r = i.find(".infos .res-jdprice"), c = 0, d = 0, l = "http://cart.jd.com/cart/dynamic/reBuyForOrderCenter.action?wids={SKUS}&nums={NUMS}", p = i.find(".btn-buy"), m = [], u = 0; a.length > u; u++)c += parseFloat(a.eq(u).attr("wmaprice")), d += parseFloat(a.eq(u).attr("wmeprice")), m.push(a.eq(u).attr("skuid"));
        "" == s || 0 >= Number(s) || 0 >= Number($(t).attr("wmeprice")) ? (o.text("\u6682\u65e0\u62a5\u4ef7"), r.text("\u6682\u65e0\u62a5\u4ef7")) : (o.hasClass("saving") || o.text("\uffe5 " + c.toFixed(2)), r.text("\uffe5 " + d.toFixed(2)), p.attr("href", l.replace("{SKUS}", m.join(",")).replace("{NUMS}", m.join(",").replace(/\d+/g, 1))), i.find(".infos .p-name span").html("" + (n - 1)))
        ...
        if (/\.css$/.test(t))a.rel = "stylesheet", a.type = "text/css", a.href = t, i.length ? i[0].appendChild(a) : e.documentElement.appendChild(a); else {
            if (s.setAttribute("type", "text/css"), s.styleSheet)s.styleSheet.cssText = t; else {
        ...
        return t > 1 && 5 > t ? "\u4e3a\u60a8\u8d2d\u4e70\u7684\u7535\u5668\u4ea7\u54c1\u5ef6\u957f\u4fdd\u4fee\u671f\u3001\u62d3\u5c55\u4fdd\u4fee\u8303\u56f4\u3002" : t > 5 && 8 > t ? "\u4e3a\u60a8\u8d2d\u4e70\u7684\u7535\u5668\u4ea7\u54c1\u7531\u4e8e\u610f\u5916\u5bfc\u81f4\u7684\u529f\u80fd\u6545\u969c\u63d0\u4f9b\u7ef4\u4fee\u6216\u66f4\u6362\u670d\u52a1\u3002" : 9 == t ? "\u4e3a\u60a8\u8d2d\u4e70\u7684800\u5143\u4ee5\u4e0b\u7684\u7535\u5668\u4ea7\u54c1\u63d0\u4f9b\u7b2c\u4e8c\u5e74\u4ee5\u6362\u4ee3\u4fee\u670d\u52a1\u3002" : 11 == t ? "\u4e13\u5bb6\u7528\u8fdc\u7a0b\u6280\u672f\u5e2e\u60a8\u89e3\u51b3\u7535\u8111\u8f6f\u4ef6\u7684\u4f7f\u7528\u95ee\u9898\u3002" : 15 == t ? "\u4e13\u5bb6\u4e0a\u95e8\u5e2e\u60a8\u5b89\u88c5\u8c03\u8bd5\u8def\u7531\u5668\uff08\u4e0d\u542b\u786c\u4ef6\uff09\u3002" : 16 == t ? "\u4e13\u5bb6\u4e0a\u95e8\u5e2e\u60a8\u5b89\u88c5\u8c03\u8bd5\u7f51\u5361\u3002" : 17 == t ? "\u4e13\u5bb6\u4e0a\u95e8\u5e2e\u60a8\u5b89\u88c5\u8c03\u8bd5\u6253\u5370\u673a\u3002" : 18 == t ? "\u4e13\u5bb6\u4e0a\u95e8\u5b89\u88c5\u7cfb\u7edf\u3001\u8f6f\u4ef6\uff0c\u8bbe\u7f6e\u7f51\u7edc\u3002(\u9700\u6b63\u7248)ID" : void 0
        ...
        var i = RegExp(t), s = pageConfig.product.specialAttrs;
        return s && s.length ? (s = s.join("^"), i.test(s) ? ("function" == typeof e && e(), !0) : !1) : !1
    }
    ...
    t.fn.jqueryzoom = function (e) {
        ...
        t(this).hover(function () {
            ...
            t(this).find("img").attr("alt", ""), 0 == t("div.zoomdiv").get().length && (t(this).after("<div class='zoomdiv'><img class='bigimg' src='" + c + "'/></div>"), t(this).append("<div class='jqZoomPup'>&nbsp;</div>")), t("div.zoomdiv").width(i.xzoom), t("div.zoomdiv").height(i.yzoom), t("div.zoomdiv").show(), i.lens || t(this).css("cursor", "crosshair"), t(document.body).mousemove(function (s) {
                mouse = new e(s);
                var c = t(".bigimg").get(0).offsetWidth, d = t(".bigimg").get(0).offsetHeight, l = "x", p = "y";
                if (isNaN(p) | isNaN(l)) {
                    var p = c / o, l = d / r;
                    t("div.jqZoomPup").width(i.xzoom / (1 * p)), t("div.jqZoomPup").height(i.yzoom / (1 * l)), i.lens && t("div.jqZoomPup").css("visibility", "visible")
                }
                xpos = mouse.x - t("div.jqZoomPup").width() / 2 - a, ypos = mouse.y - t("div.jqZoomPup").height() / 2 - n, i.lens && (xpos = a > mouse.x - t("div.jqZoomPup").width() / 2 ? 0 : mouse.x + t("div.jqZoomPup").width() / 2 > o + a ? o - t("div.jqZoomPup").width() - 2 : xpos, ypos = n > mouse.y - t("div.jqZoomPup").height() / 2 ? 0 : mouse.y + t("div.jqZoomPup").height() / 2 > r + n ? r - t("div.jqZoomPup").height() - 2 : ypos), i.lens && t("div.jqZoomPup").css({top: ypos, left: xpos}), scrolly = ypos, t("div.zoomdiv").get(0).scrollTop = scrolly * l, scrollx = xpos, t("div.zoomdiv").get(0).scrollLeft = scrollx * p
            })
        }, function () {
...
Grecommend.prototype = {init: function () {
    ...
    this.pin && (i.pin = this.pin), $.ajax({url: "http://diviner.jd.com/diviner?" + decodeURIComponent($.param(i)), dataType: "jsonp", scriptCharset: "utf-8", success: function (t) {
        e.set(t)
    }})
}, set: function (t) {
    function e(t, e) {
        for (var i = t.length / e, s = [], a = 0; i > a; a++)s.push({tabs: [], increment: null, count: e, skuids: []});
    ...
    function i(e, i) {
        var e = e;
        if (i)var s = i.split("|"), a = s[1], p = s[2];
    ...
    var s = $("#tab-hot"), a = {set: function (t, e) {
        ...
        $("#p-selected-ok").unbind("click").bind("click", function () {
            var e = $("#pop-box");
            if (e.find("#p-scroll .curr").attr("title"), 1 > $("#pop-box .curr").length)c.setTips('<p id="p-noselected">\u8bf7\u9009\u62e9\u989c\u8272</p>'); else if (1 > $("#pop-box .selected").length && !$("#p-size").hasClass("nosizes"))c.setTips('<p id="p-noselected">\u8bf7\u9009\u62e9\u5c3a\u7801</p>'); else {
                a.set(t, $("#pop-list-" + t).attr("data-res")), r.hide().clear();
                var i = $("#pop-list-" + t), s = i.attr("data-res").split("|"), n = s[0], o = s[3], d = s[4], l = i.find(".p-price img");
                l.attr("src"), i.find("input:checkbox").attr({skuid: n, wmaprice: o, wmeprice: d, checked: !0}), G.calculatePrice($("#pop-list-" + t).find("input:checkbox")[0], "#tab-hot")
            }
...
var Recommend = {init: function (t) {
    ...
    if (pageConfig.product.cbNewFittings = [pageConfig.product.skuid], pageConfig.setFittingsClk = function (t, e, i, s, a) {
        ...
        $("#newFittign-tab").prepend(s.tabs.process(t)).Jtab({event: "click", compatible: !0, currClass: "scurr"}, function (t, i, a) {
            ...
            } else $("#newFittign-tab .suits").removeAttr("style").css({width: pageConfig.wideVersion && pageConfig.compatible ? 620 : 384, overflowX: "auto"})
...
var Refresh = {init: function () {
    ...
    this.commList.find(".btn-toggle").unbind("click").bind("click", function () {
        ...
        if (i.hasClass("reply-btn"))return t.currReply = i, o = o.replace(/</g, "<").replace(/>/g, ">"), a = i.hasClass("reply-btn-lz") ? "" : a, r = {guid: s, content: o, replyId: a, nickname: n}, "" === o.replace(/\s+/, "") ? (alert("\u8bf7\u8f93\u5165\u56de\u590d\u5185\u5bb9"), void 0) : (G.checkLogin(function (e) {
    ...
    $(".wiki-pop dl"), e.each(function () {
        ...
        s.hover(function () {
            var s = $("#product-detail"), a = s.offset().left, n = s.outerWidth(), o = $(this).offset().left, r = o - a > n / 2 ? 310 : 110, c = o - a > n / 2 ? o - 300 : o - 100, d = $(this).offset().top, l = $(this).outerHeight();
            $("#des-" + e).length > 0 && ($("#des-" + e).show().css({left: c, top: d + l - 1}), $("#des-" + e).find("b").css({marginLeft: r})), t.t = setTimeout(function () {
                t.log(i)
            }, 500)
        }, function (e) {
    ...
    var e, i, s = {list: t}, a = {r1: [], r2: [], r3: [], r4: [], r5: []}, n = t.length;
    for (e = 0; n > e; e++)i = RegExp("-" + t[e].sortId + "-"), i.test("-2-3-4-44-49-") ? a.r1.push(t[e]) : i.test("-6-7-41-47-52-53-54-57-58-59-") ? a.r2.push(t[e]) : i.test("-9-48-") ? a.r3.push(t[e]) : i.test("-11-50-") ? a.r4.push(t[e]) : i.test("-15-16-17-18-45-46-") ? a.r5.push(t[e]) : a.r1.push(t[e]);
    if (s.list.length > 0) {
        this.obj.html(this.TPL.process(a)).parent().show(), pageConfig.product.hasYbInfo = !0;
...
(function () {
    ...
    var e = [
        ...
        for (var i = 0, s = e.length; s > i; i++) {
            ...
            if (!JDS.strpos(o, "360buy") && JDS.strpos(o, a) && t.isQueryParam(n))return{d: a, q: n, k: r}
        }
    };
    ...
    function t() {
        ...
        pageConfig.product.specialAttrs.join("^"), $("#choose-btn-append").addClass("disabled"), t.remove(), pageConfig.product.isWeChatStock = !0
    }
    ...
    pageConfig.product.havestock === void 0 ? (s = "", a = "http://cart.gift.jd.com/cart/addGiftToCart.action?pid=" + pageConfig.product.skuid + "&pcount=" + $("#buy-num").val() + "&ptype=1") : (s = pageConfig.product.havestock ? "" : "disabled", pageConfig.product.havestock ? (s = "", a = "http://cart.gift.jd.com/cart/addGiftToCart.action?pid=" + pageConfig.product.skuid + "&pcount=" + $("#buy-num").val() + "&ptype=1") : (s = "disabled", a = "#none")), 6980 == G.cat[2] && $('#product-detail [data-widget="tab-item"]').eq(3).hide(), $("#online-service").find("b").html("\u8054\u7cfb\u5ba2\u670d"), $("#comments-list li").eq(4).html('<a href="javascript:;">\u6709\u6652\u5355\u7684\u8bc4\u4ef7<em>(0)</em></a>'), $("body").addClass("cat-1-" + G.cat[0] + " cat-2-" + G.cat[1] + " cat-3-" + G.cat[2] + " item-" + G.sku), function () {
        ...
        JdService.init(G.sku, $("#choose-service .dd"), function (n) {
            var o = "&ybId=" + n.join(","), r = $("#buy-num").val(), c = [], d = $("#choose-btn-qiang a");
            if (n.length > 0) {
    ...
    if ($('#product-detail [data-widget="tab-item"]').find("a").addClass("toggle-link"), function () {
        ...
        var e, i, s = $(".detail-content"), a = s.find('[name^="detail-tag-id-"]'), l = [], p = $('#product-detail [data-widget="tab-item"]'), m = p.eq(0);
        return 1 > a.length ? !1 : (p.find("a").addClass("toggle-link"), o = function () {
            ...
            }), m.append('<ul class="hide sub-item">' + l.join("") + "</ul>"), e = m.children(), i = m.find("li"), m.find(".sub-item a").unbind("click").bind("click", function (t) {
                $(this).text();
                var e = $(this).attr("data-id");
                $("body,html").scrollTop($(e).offset().top - 35), i.remove("hl_red"), $(this).parent().addClass("hl_red"), t.preventDefault(), t.stopPropagation()
            })), void 0)
        }, n = function () {
        ...
        var l = new pageConfig.FN_InitSidebar, p = "98523";
...
ABTest.prototype = {test: function () {
    ...
    var t = readCookie("__jda"), e = t ? t.split(".")[1] : !1;
    if (pageConfig.scrollTopABTest = new ABTest(e, .05, 1e4), $("#o-header-2013").outerHeight() + $("#shortcut-2013").outerHeight(), $("body,html"), e) {
